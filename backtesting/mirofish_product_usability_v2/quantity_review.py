...
QUANTITY = re.compile(r"(?<![A-Za-z0-9])\d+(?:\.\d+)?\s*(?:%|％|个百分点|个?(?:自然日|交易日|工作日|季度|小时|分钟|个月)|天|日|月|年|万亿元|亿元|亿美元|万元|万美元|美元|家)")


def _quantity_key(text: str) -> str:
    return re.sub(r"\s+", "", text).replace("％", "%").replace("个自然日", "天").replace("自然日", "天").replace("个月", "月")
...
def unsupported_quantities(spec: dict, branch: dict) -> list[str]:
    ...
    if horizon.get("kind") == "calendar_days":
        basis.append(f"{horizon['value']}天")
...
def prepare_scenarios(spec: dict, result: dict) -> list[dict]:
    ...
    unit = "个自然日" if spec["horizon"]["kind"] == "calendar_days" else "个交易日"
