...
from .focused_scenarios import FOCUSED_VERSIONS, STRICT_PROMPT_VERSION, GUARDED_PROMPT_VERSION, candidate_slots, focused_prompt, normalize_focused_fields, validate_focused_grounding
...
def build_scenario_branch_prompt(
    ...
    *,
    ...
    validate_spec(spec)
    if product and product_version not in {"scenario-branch-compiler-v7", PRODUCT_PROMPT_VERSION} | FOCUSED_VERSIONS:
    ...
    branch_slots = candidate_slots(spec, financial_actions) if product and product_version in FOCUSED_VERSIONS else _build_branch_slots(
        spec,
        simulation_result,
        financial_actions,
    ...
    if product and product_version == GUARDED_PROMPT_VERSION:
        # Simulation prose may already contradict the evidence. The compact
        # product path uses the actors' actual structured choices, while raw
        # rationales and interaction transcripts remain available for audit.
        compact_input["financial_decisions"] = [
            {key: item[key] for key in ("id", "actor_id", "action_type", "decision_status", "direction", "intensity", "evidence_refs", "constraint_refs", "visibility") if key in item}
            for item in financial_actions["decisions"]
        ]
        compact_input["simulated_actions"] = []
    system = (
...
"novelty_claim":"...","confidence":0.5}],"warnings":[]}。"""
    ...
    if product:
        ...
        elif product_version in FOCUSED_VERSIONS:
            system = focused_prompt(strict=product_version in {STRICT_PROMPT_VERSION, GUARDED_PROMPT_VERSION})
            if product_version == GUARDED_PROMPT_VERSION:
                system += "\n本模式只用各方结构化选择建立条件路径。原始模拟理由和互动未作为事实输入；不得猜测被省略的内容。"
...
def build_scenario_branch_set(
    ...
    *,
    ...
    required_branch_count = _required_branch_count(financial_actions)
    focused = product and product_version in FOCUSED_VERSIONS
    if (
    ...
    for index, raw in enumerate(raw_branches, start=1):
        ...
        if product and product_version in {STRICT_PROMPT_VERSION, GUARDED_PROMPT_VERSION}:
            try:
                validate_focused_grounding(branch, spec)
            except ValueError as error:
                if product_version == STRICT_PROMPT_VERSION:
                    raise
                withheld.append({"label": branch["label"], "reason": str(error), "candidate": deepcopy(branch)})
                continue
    ...
    artifact = {
        ...
        "status": "completed" if branches and not withheld else "partial",
        "branches": branches,
        "warnings": warnings
        + normalization_warnings
        + [
    ...
    if product and product_version == GUARDED_PROMPT_VERSION:
        artifact["review"] = {"proposed_branch_count": len(raw_branches), "published_branch_count": len(branches), "withheld_branches": withheld}
        if withheld:
            artifact["warnings"].append(f"有 {len(withheld)} 条候选情景未通过依据检查，已暂存供复核，未放入观察清单。")
    if focused:
...
def append_scenario_branches_to_result(
    ...
    if "review" in branch_set:
        updated["scenario_review"] = deepcopy(branch_set["review"])
