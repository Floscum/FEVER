...
def validate_scenario_branches(
    ...
    _require(
        ...
        in {
            ...
            "scenario-branch-compiler-v10",
            "scenario-branch-compiler-v11",
        },
    ...
    if review is not None:
        _require(isinstance(review, dict) and isinstance(review.get("withheld_branches"), list), "scenario review must record withheld candidates")
        _require(review.get("published_branch_count") == len(branches), "scenario review published count mismatch")
        _require(review.get("proposed_branch_count") == len(branches) + len(review["withheld_branches"]), "scenario review proposed count mismatch")
        if review["withheld_branches"] or not branches:
            _require(artifact.get("status") == "partial", "withheld or empty scenario sets must be partial")
    ...
    for branch in branches:
        ...
        if artifact["prompt_version"] in {"scenario-branch-compiler-v10", "scenario-branch-compiler-v11"}:
            from .focused_scenarios import validate_focused_grounding
