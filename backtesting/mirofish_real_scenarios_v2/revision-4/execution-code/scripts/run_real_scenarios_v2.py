...
OUTPUT = ROOT / "artifacts/real-scenarios-v2/revision-4"
...
def freeze_code():
    names = [*sorted((ROOT / "src/fever_mirofish").glob("*.py")), *[ROOT / "scripts" / name for name in ("generate_scenario_branches.py", "run_real_scenarios.py", "run_product_iteration.py", "run_trace_ablation_experiment.py")], Path(__file__).resolve()]
    hashes = {}
    for source in names:
...
def prepare():
    if (OUTPUT / "manifest.json").exists():
        ...
        return
    manifest = {"version": 2, "revision": 4, "prepared_at": "2026-09-12", "provider": "DeepInfra", "model": live.MODEL,
                ...
                "design": "v11 now retries withheld candidates once and keeps the best valid result. Reuse revision-3 public pilot first attempts exactly, repair only its two partial cases; v7 control outputs unchanged. Then 24 fresh paired replay checks and 3 new entity-aware quick simulations. All previous calls remain in the same shared budget. No outcomes or model judging."}
    ...
    prior_pilot = ROOT / "artifacts/real-scenarios-v2/revision-3/paired"
    for entry in manifest["paired_cases"]:
        if entry["cohort"] != "public_pilot":
            continue
        for variant in ("v7", "v11"):
            prior = prior_pilot / entry["id"]
            target = OUTPUT / "paired" / entry["id"]
            output = read(prior / (variant + ".json"))
            for suffix in ("-attempts.json", "-prompt.json"):
                shutil.copy2(prior / (variant + suffix), target / (variant + suffix))
            filename = variant + ("-first-pass.json" if variant == "v11" and (output.get("artifact") or {}).get("status") == "partial" else ".json")
            write(target / filename, output)
...
def paired(stage):
    ...
    for index, entry in enumerate(selected):
        ...
        for variant in (("v7", "v11") if index % 2 == 0 else ("v11", "v7")):
            ...
            prior_output = directory / (variant + "-first-pass.json")
            if prior_output.exists():
                prior = read(prior_output)
                attempts = read(directory / (variant + "-attempts.json"))
                artifact, prior_elapsed = prior["artifact"], prior["elapsed_seconds"]
                reasons = [item["reason"] for item in artifact["review"]["withheld_branches"]]
                current_system, user = build_scenario_branch_retry_prompt(system, user, ValueError("候选需重写，补齐引用并删除无依据条件：" + "；".join(reasons)))
            for attempt in range(len(attempts), 2):
                ...
                try:
                    ...
                    candidate = build_scenario_branch_set(record["raw_response"], spec, result, actions, model_id=live.MODEL, product=True, product_version=f"scenario-branch-compiler-{variant}")
                    if artifact is None or (len(candidate["branches"]), -len(candidate.get("review", {}).get("withheld_branches", []))) > (len(artifact["branches"]), -len(artifact.get("review", {}).get("withheld_branches", []))):
                        artifact = candidate
                    if variant == "v11" and candidate["review"]["withheld_branches"] and attempt == 0:
                        raise ValueError("候选需重写，补齐引用并删除无依据条件：" + "；".join(item["reason"] for item in candidate["review"]["withheld_branches"]))
                except Exception as error:
                ...
                if artifact is not None and not record.get("error"):
                    break
            output = {"case_id": entry["id"], "variant": variant, "cohort": entry["cohort"], "artifact": artifact,
                      "elapsed_seconds": prior_elapsed + time.monotonic() - started, "attempt_count": len(attempts), "request_ids": [item["request_id"] for item in attempts]}
...
def main():
    ...
    with (DATA / "run.lock").open("a") as lock:
        ...
        if args.stage == "prepare":
            ...
            freeze_code()
            os.environ["FEVER_SCENARIO_PRODUCT_VERSION"] = "v11"
            live.execute(SimpleNamespace(label="entity-v11", case=[]))
        else:
            paired(args.stage)
