...
def main() -> int:
    ...
    parser.add_argument("--product-version", choices=("v7", "v8", "v9", "v10", "v11"), default="v7", help="v11 uses structured choices and withholds unsupported paths; v8/v9/v10 are experimental.")
    parser.add_argument(
        "--benchmark-id",
        default="engineering",
    ...
    while True:
        ...
        try:
            ...
            if args.product and args.product_version == "v11":
                if best_guarded is None or (len(branch_set["branches"]), -len(branch_set["review"]["withheld_branches"])) > (len(best_guarded["branches"]), -len(best_guarded["review"]["withheld_branches"])):
                    best_guarded = branch_set
                withheld = branch_set["review"]["withheld_branches"]
                if withheld and semantic_attempts < args.max_semantic_retries:
                    raise ValueError("候选路径需重写；保留正确内容并补齐引用，删除无依据条件：" + "；".join(item["reason"] for item in withheld))
                branch_set = best_guarded
            break
