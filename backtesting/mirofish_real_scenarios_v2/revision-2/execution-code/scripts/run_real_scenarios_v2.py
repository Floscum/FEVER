...
OUTPUT = ROOT / "artifacts/real-scenarios-v2/revision-2"
...
def prepare():
    if (OUTPUT / "manifest.json").exists():
        ...
        return
    manifest = {"version": 2, "revision": 2, "prepared_at": "2026-09-12", "provider": "DeepInfra", "model": live.MODEL,
                ...
                "design": "Revision 1 v9 failed the public pilot content review. Its six calls remain in the shared budget. Repeat v7/v10 on the same 3 public cases, then 24 existing anonymous replays; separately 3 fresh entity-aware quick simulations using v10. No outcomes or model judging."}
...
def paired(stage):
    ...
    for index, entry in enumerate(selected):
        ...
        for variant in (("v7", "v10") if index % 2 == 0 else ("v10", "v7")):
...
def main():
    ...
    with (DATA / "run.lock").open("a") as lock:
        ...
        if args.stage == "prepare":
            ...
            freeze_code()
            os.environ["FEVER_SCENARIO_PRODUCT_VERSION"] = "v10"
            live.execute(SimpleNamespace(label="entity-v10", case=[]))
        else:
            paired(args.stage)
