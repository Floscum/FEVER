...
数字相同不代表含义相同：不能把设备占比当成财务影响，也不能在缺少可比数据时
推断两个业务的利润率高低。已重述的比较口径不能再次调整。缺少依据时明确等待核对。
