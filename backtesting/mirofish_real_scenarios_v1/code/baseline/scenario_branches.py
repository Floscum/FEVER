...
def build_scenario_branch_prompt(
    ...
    *,
    ...
    validate_spec(spec)
    if product and product_version not in {"scenario-branch-compiler-v7", PRODUCT_PROMPT_VERSION}:
    ...
    branch_slots = _build_branch_slots(
        spec,
        simulation_result,
        financial_actions,
...
def build_scenario_branch_set(
    ...
    *,
    ...
    if (
        ...
        or len(raw_branches) != required_branch_count
    ):
        raise ValueError(
...
def append_scenario_branches_to_result(
    ...
    for branch in branch_set["branches"]:
        ...
        for field in ("starting_decisions", "conditional_responses", "response_semantics", "observations"):
            if field in branch:
