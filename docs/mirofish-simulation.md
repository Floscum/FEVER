...
2026-09-12 运行故障修复：假设提炼现在显示明确的完成、无新增、失败或超时状态，最长等待 45 秒；可选步骤失败会保留研究正文。推演服务的嵌套预算错误显示为可理解的提示，已关闭且缺少决策的任务提示重新推演。

配套网关为产品模式压缩角色背景预览，决策访谈保留完整事实；默认 token 额度随参与方数调整，显式配置优先。相同中国平安输入复测完成 8/8 有效决策、4 条情景，耗时 75 秒，模型估算费用 $0.02068。12 条观察条件中 8 条仍需核对，本次结果用于验证故障恢复，不代表预测有效性。
